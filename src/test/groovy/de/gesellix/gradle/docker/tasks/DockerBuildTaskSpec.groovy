...
class DockerBuildTaskSpec extends Specification {
  ...
  def "depends on tar task to archive buildContextDirectory"() {
    ...
    then:
    project.getTasksByName("tar of buildcontext for test:dockerBuild", false).size() == 1

    and:
    task.dependsOn.any { it == project.getTasksByName("tar of buildcontext for test:dockerBuild", false).first() }
  }
  ...
  def "tar of buildContextDirectory contains buildContextDirectory"() {
    ...
    then:
    def tarOfBuildcontextTask = project.getTasksByName("tar of buildcontext for test:dockerBuild", false).first()
